...
(function($) {
	...
	if (!idleStarted) {
		...
		idleStarted = true;
		$(document).off(); // do better: remove only our handler
		fn ();
	    } else {
    ...
    $( document ).ready(function () {$('.pkp_navigation_primary .pkp_nav_list').fadeTo(0,0.01);} );
	...
	$(window).resize(function () {
		...
		if (desktopMode == true) {
		    desktopMode = false;
		    console.log ('passing to tablet Mode');
		}
		...
		circleTimeout = setTimeout(initCircles, 500);
		       ...
		       && desktopMode == false) {
		desktopMode = true;
		console.log ('passing to desktop Mode');
		resetCircles();
		initCircles();
	    }
	});
	...
	if ($('.pkp_page_index').length != 0 || $('.pkp_page_404').length != 0) {
		...
		case 6:
		    ...
		    break;
		case 7:
		    var ww = ($(window).width() - parseFloat(w))/2;
		    img.system(dynBounce2D
			       ([5, -20, 10],
				[-ww, ww,
				 -100, $(window).height() - 100 - parseFloat(h)]
				,dynGravity  // experimental
			       ),
			       // TODO remplacer 100 par un calcul
			       p, 20000);
		    break;
		}
	    }
