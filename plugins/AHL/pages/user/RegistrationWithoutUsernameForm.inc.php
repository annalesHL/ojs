...
class RegistrationWithoutUsernameForm extends Form {
	...
	function __construct($site) {
		parent::__construct('frontend/pages/userRegisterWithoutUsername.tpl');
	...
	function execute($request) {
		...
		$username = Validation::suggestUsername($this->getData('firstName'), $this->getData('lastName'));
		...
		$user->setPassword(Validation::encryptCredentials($username, $this->getData('password')));
		...
		if ($requireValidation) {
			...
			$mail->assignParams(array(
				...
				'activateUrl' => $request->url($context->getPath(), 'user', 'activateUser', array($username, $accessKey))
			));
